    ...
    if (assignedKey) {
        ...
        if (isCustomTag) {
            ...
            if (assignedKey.type === "Literal") {
                ...
                idExpression = builder.binaryExpression(builder.literal("@"), "+", assignedKey);
            }
        ...
        if (parentForKey) {
            idExpression = builder.binaryExpression(idExpression, "+", parentForKey);
            nestedIdExpression = builder.binaryExpression(nestedIdExpression, "+", parentForKey);
        }
    ...
    this.componentIdInfo = {
        ...
        createIdVarNode: function () {
            ...
            this.idVarNode = builder.vars([{
                id: idVarName,
                init: builder.functionCall(builder.memberExpression(builder.identifier("__component"), builder.identifier("_i_")), [idExpression])
            }]);

            this.idExpression = idExpression = idVar;

            this.nestedIdExpression = nestedIdExpression = builder.concat(builder.literal("#"), idVar);
...
const getParentForKeyVar = (el, transformHelper) => {
    ...
    const keyExpression = getChildKey(parentFor) || createIndexKey(parentFor, transformHelper);
    const bracketedKeyExpression = builder.binaryExpression(builder.literal("["), "+", builder.binaryExpression(keyExpression, "+", builder.literal("]")));
...
const createIndexKey = (forNode, transformHelper) => {
    ...
    if (parentForKey) {
        keyExpression = builder.binaryExpression(keyExpression, "+", parentForKey);
    }
