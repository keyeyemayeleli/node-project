...
class CustomTag extends HtmlElement {
    ...
    buildInputProps(codegen, additionalAttrs) {
        ...
        function handleAttr(attrName, attrValue, attrDef) {
            ...
            if (attrDef.dynamicAttribute) {
                ...
                //
                if (tagDef.isDynamicTag !== true && (attrDef.removeDashes === true || attrDef.preserveName === false)) {
                ...
                } else if (attrDef.preserveName === true || tagDef.isDynamicTag === true) {
                    propName = attrName;
                } else {
                    propName = removeDashes(attrName);
                }
            ...
            if (parentPropName) {
                let parent = explicitAttrs[parentPropName] || (explicitAttrs[parentPropName] = {});
        ...
        if (tagDef.forEachAttribute) {
            ...
            tagDef.forEachAttribute(function (attrDef) {
                if (attrDef.hasOwnProperty("defaultValue")) {
                    handleAttr(attrDef.name, context.builder.literal(attrDef.defaultValue), attrDef);
                }
            });
        }
        ...
        this.forEachAttribute((attr, i) => {
            ...
            if (attr.spread) {
                let isFirstOfMany = i === 0 && (this._hasDynamicNestedTags || this.attributes.length > 1);
                ...
                let attrDef = isDynamicTag ? {} : attr.def || context.taglibLookup.getAttribute(tagName, attrName) || tagDef.getAttribute(attr.name);
        ...
        inputProps = attrs.length > 1 ? builder.functionCall(context.helper("assign"), attrs) : attrs[0];
    ...
    resolveTagDef(context) {
        ...
        if (!tagDef) {
            if (this.tagName && this.tagName.startsWith("@")) {
                ...
                if (!parentCustomTag) {
                    throw new Error("Invalid usage of the <" + this.tagName + "> nested tag. Tag not nested within a custom tag.");
                }
    ...
    addNestedTag(nestedTag) {
        ...
        let byNameArray = this._foundNestedTagsByName[tagName] || (this._foundNestedTagsByName[tagName] = []);
    ...
    generateRenderNode(codegen, tagDef, inputProps, parentCustomTag) {
        ...
        if (tagDef.template) {
            ...
            if (isNestedTag) {
                ...
                tagArgs = [this.tagNameExpression, inputProps, builder.identifierOut()];
            } else {
    ...
    generateRenderBodyCode(codegen, body) {
        ...
        if (hasBody) {
            if (tagDef.bodyFunction) {
                ...
                return builder.functionDeclaration(bodyFunctionName, bodyFunctionParams, body);
            } else {
    ...
    generateCode(codegen) {
        ...
        if (isNestedTag) {
            ...
            if (hasBody && context.isServerTarget() && !parentCustomTag.isFlagSet(HAS_RENDER_BODY_PROP_ADDED)) {
                ...
                let hasRenderBodyKey = context.addStaticVar("hasRenderBodyKey", builder.functionCall(builder.memberExpression(builder.identifier("Symbol"), builder.identifier("for")), [builder.literal("hasRenderBody")]));
            ...
            if (checkIfNestedTagCanBeAddedDirectlyToInput(this, parentCustomTag)) {
                ...
                let renderBody = hasBody ? builder.renderBodyFunction(body, [builder.identifier("out")].concat(params)) : null;
                ...
                if (tagDef.isRepeated) {
                    ...
                    if (currentValue) {
                        ...
                        parentCustomTag.setAttributeValue(tagDef.targetProperty, builder.arrayExpression([inputProps]));
                    }
                } else {
        ...
        if (renderBody && bodyOnlyIf) {
            ...
            renderBodyFunctionVar = builder.var(renderBodyFunctionVarIdentifier, renderBody);
        ...
        if (bodyOnlyIf && renderBodyFunctionVar) {
            let ifStatement = builder.ifStatement(bodyOnlyIf, [builder.functionCall(renderBodyFunctionVarIdentifier, [builder.identifierOut()])], builder.elseStatement([renderTagNode]));
