...
module.exports = function (node, codegen, vdomUtil) {
    ...
    if (properties) {
        Object.keys(properties).forEach(key => properties[key] = codegen.generateCode(properties[key]));
    }
    ...
    if (bodyOnlyIf) {
        ...
        var startIf = builder.ifStatement(builder.negate(bodyOnlyIf), [htmlElVDOM]);

        var endIf = builder.ifStatement(builder.negate(bodyOnlyIf), [new EndElementVDOM()]);
