...
module.exports = function renderCallToDynamicTag(ast, context) {
    ...
    if (argsLength <= 2) {
        ...
        tagName = builder.functionDeclaration(null, [builder.identifier("out")], [ast]);
    }

    const el = builder.htmlElement(undefined, tagAttrs, undefined, undefined, true, true);
...
function toAttributesOrSpread(val) {
    if (!val || val.type === "Literal" && val.value === null || val.type === "Identifier" && val.name === "undefined") {
        return [];
    }

    if (val.type === "ObjectExpression" && val.properties.every(prop => !prop.computed)) {
