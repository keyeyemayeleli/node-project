...
module.exports = function nodeFactory(elNode, context) {
    ...
    if (!attributes) {
        context.addError("Invalid <var> tag. Argument is missing. Example; <var x=123 />");
        return elNode;
    }
    ...
    if (lastChild && lastChild.type === "Text" && /\s+$/.test(lastChild.argument.value)) {
    ...
    const scriptlets = elNode.attributes.map(attr => {
        ...
        if (!isValidJavaScriptVarName(name)) {
            hasError = true;
            context.addError("Invalid JavaScript variable name: " + name, "INVALID_VAR_NAME");
            return;
        }

        return builder.scriptlet({
            value: `var ${val == null ? name : `${name} = ${printJS(attr.value, context)}`}`
        });
    });
