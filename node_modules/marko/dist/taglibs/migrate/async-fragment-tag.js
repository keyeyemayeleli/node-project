...
module.exports = function migrator(oldNode, context) {
    ...
    context.deprecate(`The "<${oldTag}>" tag is deprecated. Please use "<${newTag}>" instead. See: https://github.com/marko-js/marko/wiki/Deprecation:-async-fragment`);

    if (oldTag == "async-fragment" /* new: <await> */) {
            if (!attributes || !attributes.length) {
                context.addError('Invalid <aync-fragment> tag. Argument is missing. Example; <async-fragment data-provider=data.userInfo var="userInfo" />');
                return oldNode;
            }
            ...
            if (!context.util.isValidJavaScriptIdentifier(varName)) {
                context.addError('Invalid <aync-fragment> tag. Argument\'s variable name should be a valid JavaScript identifier. Example: user, as in <async-fragment data-provider=data.userInfo var="userInfo" />');
                return;
            }
